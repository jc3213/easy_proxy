...
function menuEventSave() {
    ...
    chrome.runtime.sendMessage({ system: 'storage_update', params: easyStorage });
}
...
importEntry.addEventListener('change', (event) => {
    ...
    reader.onload = (event) => {
        ...
        chrome.runtime.sendMessage({ system: 'storage_update', params });
    };
...
chrome.runtime.sendMessage({ system: 'storage_fetch' }, (storage) => {
    storageDispatch(storage);
    actionPane.classList.add(storage.action);
});

function profileExport(id) {
    chrome.runtime.sendMessage({ system: 'profile_fetch', params: id }, (pac_script) => {
